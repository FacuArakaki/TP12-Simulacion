...
const portada = [
  new Paragraph({
    ...
    children: [t("Optimización de costos en logística de última milla mediante una red de lockers inteligentes", { b: true, size: S16 })],
  }),
  new Paragraph({
    alignment: AlignmentType.CENTER, spacing: { after: 80 },
    children: [t("[Autores: completar en la versión final — la versión para evaluación debe ser ciega]", { b: true, size: S14 })],
  }),
  new Paragraph({
...
c.push(tabla(
  ...
  [
    ...
    ["TMP", "Parámetro de control (3 días)", "min"],
    ["IPC", "Parámetro de control (1 día)", "min"],
  ],
  [1150, 2000, 750]
));
...
c.push(p([t("Cada configuración se evaluó con treinta réplicas independientes, informando la media y el semiancho del intervalo de confianza del 95 %. Para comparar configuraciones se emplearon números aleatorios comunes: el generador mantiene cuatro flujos aleatorios separados —llegadas, tamaños, retiros y devoluciones— de modo que dos configuraciones con la misma semilla enfrentan exactamente la misma secuencia de eventos. De este modo, las diferencias observadas entre diseños se deben al diseño y no al ruido de la simulación, lo que reduce sensiblemente la varianza de la comparación.")]));
c.push(p([t("La optimización se realizó mediante un barrido exhaustivo sobre la grilla de configuraciones (CLTC, CLTM, CLTG) para cada escenario, seleccionando la de menor CPE. El horizonte de simulación es de un año, por lo que el período transitorio inicial —el sistema comienza con todos los compartimentos vacíos— resulta despreciable frente a la longitud de la corrida.")]));
...
c.push(cap("Tabla 3. Configuración óptima por escenario."));
c.push(tabla(
...
// ---------------- Discusión ----------------
c.push(H("Discusión"));
c.push(p([t("Los resultados confirman que el dimensionamiento de una red de lockers constituye un problema de optimización con solución interior. La forma de U del costo responde a la tensión entre dos componentes de signo opuesto: un costo fijo que crece con la capacidad instalada y un conjunto de penalizaciones que decrece con ella. La consecuencia práctica es que ni la estrategia de instalar la mayor capacidad posible ni la de minimizar la inversión inicial resultan convenientes.")]));
...
c.push(p([t("Un hallazgo relevante es la planicie de la región óptima. Varias configuraciones presentan costos estadísticamente equivalentes en el entorno del mínimo, lo que otorga al operador un grado de libertad adicional para decidir según criterios no contemplados en la función de costos, tales como restricciones de espacio físico o disponibilidad de módulos comerciales.")]));
c.push(p([t("El análisis de sensibilidad aporta dos conclusiones de distinta naturaleza. La primera es metodológica: identificó a la amortización de los compartimentos como el parámetro determinante, lo que motivó reemplazar el supuesto inicial por un valor derivado de un costo total de propiedad publicado [4]. El parámetro de mayor peso pasó así a ser el mejor respaldado del modelo. La segunda es sustantiva: el diseño óptimo se mantiene estable ante variaciones de ±30 % en los costos, de modo que la recomendación no depende de la precisión de los parámetros menos documentados.")]));
c.push(p([t("El trabajo presenta limitaciones que conviene explicitar. Se modela un único punto de lockers, por lo que no se capturan efectos de red ni la posibilidad de derivar paquetes entre puntos cercanos. La tasa de arribos proviene de una operación que abarca varias zonas y se escaló para representar un barrio, lo que constituye un supuesto de alcance. El costo total de propiedad utilizado no incluye el alquiler del espacio que hospeda el locker, por lo que la amortización empleada podría subestimar el costo real. Por último, corresponde distinguir entre dos tipos de incertidumbre. La incertidumbre económica resultó inocua: el diseño óptimo no se modifica ante variaciones de ±30 % en los costos. La incertidumbre de comportamiento, en cambio, sí es determinante: el tiempo de retiro desplaza el óptimo de 30 a 50 compartimentos chicos dentro del rango evaluado. Esto no invalida el modelo, sino que precisa su alcance: la metodología y la estructura de costos son robustas, mientras que el dimensionamiento absoluto debe recalibrarse una vez que exista operación real de la que medir el comportamiento de retiro.")]));
...
c.push(p([t("El análisis de sensibilidad establece que la amortización de los compartimentos es el componente de costo determinante y que, no obstante, el diseño óptimo permanece estable ante variaciones de ±30 % en los parámetros de costo. El análisis sobre los parámetros de comportamiento muestra en cambio que el tiempo de retiro escala la capacidad requerida de forma aproximadamente proporcional, desplazando el óptimo de 30 a 50 compartimentos chicos dentro del rango evaluado. La recomendación es por lo tanto robusta frente a la incertidumbre económica, mientras que el dimensionamiento absoluto queda condicionado al comportamiento de retiro de los usuarios, magnitud que sólo podrá medirse una vez desplegada la red.")]));

// ---------------- Agradecimientos ----------------
c.push(Hs("Agradecimientos"));
c.push(p([t("Los autores agradecen al Ing. David Mammana por el seguimiento y las observaciones realizadas durante la definición del modelo, y a [nombre del contacto] de [empresa] por facilitar los registros de operación que hicieron posible la caracterización de la demanda.", { size: S10 })]));
...
c.push(p([t("[Nombre y Apellido. Universidad Tecnológica Nacional, Facultad Regional Buenos Aires. Medrano 951, C1179AAQ, Ciudad Autónoma de Buenos Aires, Argentina. correo@ejemplo.com]", { i: true, size: S10 })]));
